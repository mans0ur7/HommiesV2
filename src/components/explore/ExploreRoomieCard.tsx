  ...
  return (
    ...
    >
      <div className="relative">
        <Avatar className="w-20 h-20 md:w-28 md:h-28 mb-2 ring-2 md:ring-4 ring-secondary/30 group-hover:ring-secondary group-focus-visible:ring-ring transition-all duration-300 group-hover:shadow-lg group-hover:scale-105">
          {image ? (
            <AvatarImage src={image} alt={`Profilbillede af ${name}`} className="object-cover transition-transform duration-300" />
          ) : null}
          <AvatarFallback className="bg-gradient-to-br from-secondary/20 to-secondary/5">
            <User className="w-8 h-8 md:w-10 md:h-10 text-muted-foreground" aria-hidden="true" />
          </AvatarFallback>
        </Avatar>
        
        {/* Online/active indicator - decorative */}
        <div className="absolute bottom-2 right-0 md:bottom-3 md:right-0 w-4 h-4 md:w-5 md:h-5 bg-green-500 rounded-full border-2 border-background shadow-sm" aria-hidden="true" />
      </div>
